...
class CacheTipTests(unittest.TestCase):
    ...
    def test_low_cache_hit_emits_tip(self):
        ...
        self.assertTrue(any(t["category"] == "缓存" for t in tips))

    def test_healthy_cache_no_tip(self):
        for i in range(10):
            self._ins(f"2026-04-15T00:00:0{i}Z", "projY", 1_000_000, 50)
        tips = cache_discipline_tips(self.db, today_iso="2026-04-19T00:00:00")
        self.assertFalse(any(t["category"] == "缓存" for t in tips))
...
class RepeatTipTests(unittest.TestCase):
    ...
    def test_repeated_file_and_bash_emit_tips(self):
        ...
        self.assertIn("重复文件", cats)
        self.assertIn("重复命令", cats)
...
class RightSizeTests(unittest.TestCase):
    ...
    def test_short_gpt_5_6_sol_turns_flagged(self):
        with connect(self.db) as c:
            for i in range(10):
                c.execute("INSERT INTO messages (uuid, session_id, project_slug, type, timestamp, model, input_tokens, output_tokens, cache_read_tokens, cache_create_5m_tokens, cache_create_1h_tokens, is_sidechain) VALUES (?, 's','p','assistant','2026-04-18T00:00:00Z','gpt-5.6-sol', 1000000, 200, 0, 0, 0, 0)", (f"a{i}",))
            c.commit()
        tips = right_size_tips(self.db, today_iso="2026-04-19T00:00:00")
        self.assertTrue(any("gpt-5.6-terra" in t["title"] for t in tips))

    def test_short_opus_turns_are_not_priced(self):
        with connect(self.db) as c:
            for i in range(10):
                c.execute("INSERT INTO messages (uuid, session_id, project_slug, type, timestamp, model, input_tokens, output_tokens, is_sidechain) VALUES (?, 's','p','assistant','2026-04-18T00:00:00Z','claude-opus-4-7', 1000000, 200, 0)", (f"opus-{i}",))
            c.commit()
        tips = right_size_tips(self.db, today_iso="2026-04-19T00:00:00")
        self.assertFalse(any("Opus" in t["title"] or "Sonnet" in t["title"] for t in tips))

    def test_mixed_historical_prices_are_used_for_source_and_target(self):
        with connect(self.db) as c:
            for i in range(10):
                timestamp = (
                    "2026-07-30T16:59:59Z" if i < 5 else "2026-07-30T17:00:00Z"
                )
                c.execute(
                    ...
                    "VALUES (?, 's','p','assistant',?,'gpt-5.6-terra', "
                    "100000, 200, 0, 0, 0, 0)",
                    (f"terra-{i}", timestamp),
                )
            c.commit()

        tips = right_size_tips(self.db, today_iso="2026-08-01T00:00:00Z")
        tip = next(t for t in tips if "gpt-5.6-luna" in t["title"])
        self.assertIn("$2.28", tip["body"])
        self.assertIn("$0.61", tip["body"])
...
class OutlierTests(unittest.TestCase):
    ...
    def test_giant_tool_result_flagged(self):
        ...
        self.assertTrue(any(t["category"] == "工具输出过大" for t in tips))
