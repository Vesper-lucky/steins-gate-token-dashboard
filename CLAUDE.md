...
**Divergence Ledger** - 世界线 Token 记录仪 for local Codex/Claude Code token usage, costs, and session history.
...
Env vars: `PORT` (default 8081), `HOST` (default 127.0.0.1), `CLAUDE_PROJECTS_DIR`, `TOKEN_DASHBOARD_DB`. Pricing lives in `pricing.json`. See README.md § Environment variables for details.
...
curl http://127.0.0.1:8081/api/overview   # sanity-check an endpoint
```
